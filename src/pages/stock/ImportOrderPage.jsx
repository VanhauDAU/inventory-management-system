...
const DEMO_ORDERS = [
  { id: 1, transaction_code: 'NK-2026-001', warehouse: { name: 'Kho Hà Nội' }, reason: 'Nhập hàng định kỳ', note: '', created_by_name: 'Admin', created_at: '2026-06-12T08:30:00Z', items: [{ product_name: 'Laptop Dell', quantity: 3, unit_price: '15500000', total_amount: '46500000' }] },
  { id: 2, transaction_code: 'NK-2026-002', warehouse: { name: 'Kho TP.HCM' }, reason: 'Bổ sung hàng tồn kho', note: 'Ưu tiên nhập sớm', created_by_name: 'Admin', created_at: '2026-06-10T09:00:00Z', items: [{ product_name: 'Bàn phím Keychron', quantity: 10, unit_price: '2190000', total_amount: '21900000' }] },
]
...
export default function ImportOrderPage() {
  ...
  async function fetchOrders(p = 1) {
    ...
    try {
      const res = await api.get(`/stock-transactions/?transaction_type=import&ordering=-created_at&page=${p}`)
      const data = res.data
  ...
  async function handleCreate(e) {
    ...
    if (items.some((it) => !it.product || !it.quantity || !it.unit_price)) { setFormError('Vui lòng điền đầy đủ thông tin từng sản phẩm'); return }
    setSaving(true); setFormError('')
    try {
      await api.post('/stock-transactions/', { ...form, transaction_type: 'import', items: items.map((it) => ({ product: it.product, quantity: Number(it.quantity), unit_price: it.unit_price })) })
    ...
    <div className="stock-page">
      <div className="stock-header">
        <div>
          ...
          <p>Tạo và quản lý các phiếu nhập hàng vào kho</p>
        </div>
        <button type="button" className="stock-add-btn" onClick={openCreate}>
      ...
      <div className="stock-card">
        <div className="stock-card-head">
          <span className="stock-count">{total} phiếu nhập kho</span>
        ...
        {loading ? (
          ...
          <div className="stock-state"><span style={{ fontSize: '2rem' }}>📥</span><p>Chưa có phiếu nhập nào.</p></div>
        ) : (
              ...
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id}>
                    ...
                    <td>
                      <button type="button" className="stock-btn-view" onClick={() => setDetailOrder(o)}>Xem</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            <form onSubmit={handleCreate} className="stock-form">
              <div className="stock-form-row">
                <div className="stock-form-group">
                  <label>Kho nhập <span className="required">*</span></label>
                ...
                <div className="stock-form-group">
                  ...
                  <input value={form.reason} onChange={(e) => setForm((p) => ({ ...p, reason: e.target.value }))} placeholder="VD: Nhập hàng định kỳ" />
                </div>
              </div>
              <div className="stock-form-group">
                <label>Ghi chú</label>
                <textarea value={form.note} onChange={(e) => setForm((p) => ({ ...p, note: e.target.value }))} rows={2} placeholder="Ghi chú thêm..." />
              </div>

              {/* Danh sách sản phẩm */}
              <div>
                <div className="stock-items-label">Danh sách sản phẩm <span className="required">*</span></div>
                ...
                <button type="button" className="stock-add-item-btn" onClick={addItem} style={{ marginTop: '0.5rem' }}>
                  + Thêm sản phẩm
                </button>
              </div>
              ...
              <div className="stock-form-actions">
                ...
                <button type="submit" className="stock-btn-save" disabled={saving}>{saving ? 'Đang lưu...' : 'Tạo phiếu nhập'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {detailOrder && (
        <div className="stock-modal-backdrop" role="presentation" onClick={() => setDetailOrder(null)}>
          <div className="stock-modal" role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
            <div className="stock-modal-header">
              <h3>📋 Chi tiết phiếu nhập — {detailOrder.transaction_code}</h3>
            ...
            <div className="stock-detail-section">
              ...
              {detailOrder.note && <p style={{ color: '#64748b', fontSize: '0.875rem', marginBottom: '1rem' }}>{detailOrder.note}</p>}
