...
const PAGE_SIZE = 10
const DEMO_ORDERS = [
  { id: 1, transaction_code: 'DC-2026-001', warehouse: { name: 'Kho Hà Nội' }, reason: 'Kiểm kê tháng 6', note: '', created_by_name: 'Admin', created_at: '2026-06-10T10:00:00Z', items: [{ product_name: 'Ổ cứng SSD Samsung', quantity: 2, unit_price: '1890000', total_amount: '3780000' }] },
]
...
export default function AdjustmentOrderPage() {
  ...
  async function fetchOrders(p = 1) {
    ...
    try {
      const res = await api.get(`/stock-transactions/?transaction_type=adjustment&ordering=-created_at&page=${p}`)
      const data = res.data
  ...
  async function handleCreate(e) {
    ...
    if (items.some((it) => !it.product || !it.quantity || !it.unit_price)) { setFormError('Điền đầy đủ thông tin'); return }
    setSaving(true); setFormError('')
    try {
      await api.post('/stock-transactions/', { ...form, transaction_type: 'adjustment', items: items.map((it) => ({ product: it.product, quantity: Number(it.quantity), unit_price: it.unit_price })) })
  ...
  return (
    <div className="stock-page">
      <div className="stock-header">
        <div>
          <h2>Phiếu điều chỉnh kho</h2>
          <p>Điều chỉnh số lượng tồn kho khi kiểm kê phát sinh chênh lệch</p>
        </div>
        <button type="button" className="stock-add-btn amber" onClick={() => { setForm({ warehouse: '', reason: '', note: '' }); setItems([{ product: '', quantity: 1, unit_price: '' }]); setFormError(''); setShowCreate(true) }}>
      ...
      <div className="stock-card">
        <div className="stock-card-head">
          <span className="stock-count">{total} phiếu điều chỉnh</span>
        ...
        {loading ? (
          ...
          <div className="stock-state"><span style={{ fontSize: '2rem' }}>🔧</span><p>Chưa có phiếu điều chỉnh nào.</p></div>
        ) : (
          <div className="stock-table-wrap">
            <table className="stock-table">
              <thead><tr><th>Mã phiếu</th><th>Kho</th><th>Lý do</th><th>Người tạo</th><th>Ngày tạo</th><th>Thao tác</th></tr></thead>
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id}>
            ...
            <form onSubmit={handleCreate} className="stock-form">
              <div className="stock-form-row">
                ...
                <div className="stock-form-group">
                  ...
                  <input value={form.reason} onChange={(e) => setForm((p) => ({ ...p, reason: e.target.value }))} placeholder="VD: Kiểm kê định kỳ" />
                </div>
              </div>
              <div className="stock-form-group">
                <label>Ghi chú</label>
                <textarea value={form.note} onChange={(e) => setForm((p) => ({ ...p, note: e.target.value }))} rows={2} placeholder="Mô tả chênh lệch..." />
              </div>
              <div>
                <div className="stock-items-label">Sản phẩm điều chỉnh <span className="required">*</span></div>
                <div className="stock-item-rows">
                  {items.map((it, i) => (
                    <div key={i} className="stock-item-row">
                      ...
                      <input type="number" min="1" value={it.quantity} onChange={(e) => updateItem(i, 'quantity', e.target.value)} placeholder="SL điều chỉnh" />
              ...
              <div className="stock-form-actions">
                ...
                <button type="submit" className="stock-btn-save amber" disabled={saving}>{saving ? 'Đang lưu...' : 'Tạo phiếu điều chỉnh'}</button>
              </div>
            </form>
          </div>
        </div>
